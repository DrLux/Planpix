...
class GymEnv():
    ...
    def __init__(self, env_name, seed, max_episode_length):
        ...
        self._env = wrappers.Monitor(self._env, '.', video_callable=False ,force=True)
        self._env.seed(seed)
        self.observation_space = Box(low=0,high=255,shape=self.obs_size + (3,))
        self.max_episode_length = max_episode_length
        self.t = 0 #counter of total steps performed
    ...
    def step(self,action):
        ...
        #self._env.viewer.window.dispatch_events()
        self.t += 1
        done = done or self.t >= self.max_episode_length
        obs = process_frame(frame)
