...
class ControlSuiteEnv():
    def __init__(self, env_name, seed, max_episode_length, bit_depth, frame_as_state):
    ...
    def reset(self):
        ...
        if self.frame_as_state:
            state = _images_to_observation(self._env.physics.render(height=64, width=64, camera_id=0), self.bit_depth) 
        else:
            state = torch.tensor(np.concatenate([np.asarray([obs]) if isinstance(obs, float) else obs for obs in state.observation.values()], axis=0), dtype=torch.float32).unsqueeze(dim=0)
        return state
