...
class Initializer():
  def __init__(self):  
      ...
      self.metrics = {'steps': [], 'episodes': [], 'train_rewards': [], 'test_episodes': [], 'test_rewards': [], 'actor_loss': [], 'critic_loss': []}    
      ...
      self.env = ControlSuiteEnv(self.parms.env_name, self.parms.seed, self.parms.max_episode_length, self.parms.bit_depth, self.parms.frame_as_state)
  ...
  def init_exp_rep(self):
    ...
    for s in tqdm(range(1, self.parms.num_init_episodes +1)):
      ...
      while not done:
        ...
        self.D.append(observation, action, done, next_observation, reward)
