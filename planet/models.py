...
class TransitionModel(jit.ScriptModule):
    ...
    def __init__(self, belief_size, state_size, action_size, hidden_size, embedding_size, activation_function='relu', min_std_dev=0.1):
        ...
        self.fc_embed_belief_posterior = nn.Linear(belief_size + embedding_size, hidden_size)# +1 is the reward 
        self.fc_state_posterior = nn.Linear(hidden_size, 2 * state_size)

    @jit.script_method
    def forward(self, prev_state:torch.Tensor, actions:torch.Tensor, prev_belief:torch.Tensor, observations:Optional[torch.Tensor]=None, nonterminals:Optional[torch.Tensor]=None, rewards:Optional[torch.Tensor]=None) -> List[torch.Tensor]:
...
class ObservationModel(jit.ScriptModule): #decoder
  __constants__ = ['embedding_size']
